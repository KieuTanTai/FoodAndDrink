...
namespace ProjectShop.Server.Core.Interfaces.IRepositories.IEntityRepositories
{
        ...
        IBaseGetByCreatedAndLastUpdatedDate<Account>
    {
        ...
        Task<IEnumerable<Account>> GetManyByUserNamesAsync(IEnumerable<string> userNames, uint? fromRecord = 0, uint? pageSize = null, CancellationToken cancellationToken = default);
        Task<IEnumerable<Account>> SearchByUserNameContainsAsync(string searchTerm, uint? fromRecord = 0, uint? pageSize = null, CancellationToken cancellationToken = default);

        // Query by AccountStatus
        Task<IEnumerable<Account>> GetByStatusAsync(bool status, uint? fromRecord = 0, uint? pageSize = null, CancellationToken cancellationToken = default);
